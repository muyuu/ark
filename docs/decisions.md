...
デスクトップまたは WSL、サーバはその裏返し。

層を累積（最小 ⊂ 開発 ⊂ デスクトップ）にしなかったのは、**サーバにだけ要る物**の置き場が無くなるため。
サーバで tmux が要るのは切断されても作業を続けるためで、デスクトップの理由とは違う。用途別の層を
排他にすると、それぞれの層が「どの用途のための宣言か」を素直に表せる。
...
## dotfiles は宣言していないツールに依存しない

`config/` は全マシンへ同じものを配る。パッケージの層と違って dotfiles には層を作っていない——symlink は
置いてあるだけならコストも害も無く、層を分けるとリンクの仕組みが複雑になるため。
...
巻き添えにすると、直すために毎回その 1 つを外す必要が出る。だから失敗は記録して先へ進み、実行の
最後にまとめて出す。

distro の package manager だけは事情が違い、まとめて 1 コマンドで渡すと 1 つの書き間違いで全滅する。
まず全部で試し、落ちたときだけ 1 つずつ入れ直して犯人を特定する。通常時のコストを増やさずに切り分け
られる。

一方でオーケストレーション（keel）から呼ぶときは、部分的な失敗も知りたい。対話での利用と要求が逆に
なるので、`--strict` で切り替える形にした。既定は「止めない」——ark を直に叩くのは人だから。
