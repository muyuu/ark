...
気づける。bootstrap から流すときも同じで、引数はそのまま install へ渡る。
...
2 つのコマンドが表裏になっている。どちらも何も直さず、判断材料を出すだけ。

| コマンド     | 見るもの                                                                                                              |
| ------------ | --------------------------------------------------------------------------------------------------------------------- |
| `ark doctor` | **宣言にあるのに実機で効いていない**物（リンク切れ、未ビルド、未取得の overlay、鍵の欠け、PM が知らないパッケージ名） |
| `ark audit`  | **実機にあるのに宣言に無い**物（手で入れたまま書き忘れたパッケージ）                                                  |

新しいマシンや久しぶりのマシンで「何かおかしい」と思ったら `ark doctor` から。

```
⚠️ 2 件のズレが見つかりました:
[dotfiles]
  /home/me/.vimrc: リンクがありません → ark link-dotfiles
[command]
